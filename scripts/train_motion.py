...
from .export_utils import export_embeddings
...
def main(h: HParams) -> None:
    ...
    # 1) Load and normalize raw training embeddings
    X_raw, t_raw = load_or_generate(h.data_path)  # X_raw: [T, N, D]
    ...
    # 6) Export low-dimensional embeddings for training set
    export_embeddings(
        f=f,
        X_raw=X_raw,
        t_raw=t_raw,
        stats=stats,
        device=h.device,
        out_npz=os.path.join(h.ckpt_dir, f"Y_ld_{h.norm_mode}.npz"),
        per_epoch_dir=os.path.join(h.ckpt_dir, f"embeddings_{h.norm_mode}"),
        batch_n=2048,
    )
